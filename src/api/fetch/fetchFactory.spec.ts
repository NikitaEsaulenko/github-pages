...
import { fetchFactory, type Request } from './fetchFactory'
...
describe('fetchFactory', () => {
  ...
  const fetchClient = fetchFactory(baseUrl)
  ...
  it('should make a request with the correct URL and return a successful response', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(`${baseUrl}/test?id=123`, {
      body: undefined,
      headers: {
        Authorization: 'Bearer token',
  ...
  it('should return an error response when fetch fails', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(`${baseUrl}/error`, {
      body: undefined,
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  it('should include query parameters in the URL when present', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(`${baseUrl}/data?filter=active`, {
      body: undefined,
      headers: {
        'Content-Type': 'application/json',
      },
